...
void toEpoll(net::FdEvent::ptr fd_event, int events) {
  ...
  if (events & net::IOEvent::READ) {
    LOG(DEBUG) << "fd:[" << fd_event->getFd()
               << "], register read event to epoll";
    fd_event->setCallBack(net::IOEvent::READ, [cur_cor, fd_event]() {
  ...
  if (events & net::IOEvent::WRITE) {
    ...
    fd_event->setCallBack(net::IOEvent::WRITE,
                          [cur_cor]() { common::Coroutine::Resume(cur_cor); });
...
ssize_t read_hook(int fd, void* buf, size_t count) {
  ...
  net::FdEvent::ptr fd_event =
      net::FdEventContainer::GetFdContainer()->getFdEvent(fd);
  ...
  // because reactor should always care read event when a connection sockfd was
  // created so if first call sys read, and read return success, this fucntion
  // will not register read event and return for this connection sockfd, reactor
  // will never care read event
...
int accept_hook(int sockfd, struct sockaddr* addr, socklen_t* addrlen) {
  ...
  net::FdEvent::ptr fd_event =
      net::FdEventContainer::GetFdContainer()->getFdEvent(sockfd);
  if (fd_event->getReactor() == nullptr) {
    fd_event->setReactor(net::Reactor::GetReactor());
  }
...
int connect_hook(int sockfd, const struct sockaddr* addr, socklen_t addrlen) {
  ...
  if (n == 0) {
    ...
    LOG(DEBUG) << "connect error and errno is't EINPROGRESS, errno=" << errno
  ...
  net::TimerEvent::ptr event =
      std::make_shared<net::TimerEvent>(g_max_timeout, false, timeout_cb);
  ...
  LOG(DEBUG) << "connect error and errno=" << errno
             << ", error=" << strerror(errno);
  return -1;
}
...
unsigned int sleep_hook(unsigned int seconds) {
  ...
  net::TimerEvent::ptr event =
      std::make_shared<net::TimerEvent>(1000 * seconds, false, timeout_cb);
...
void setMaxTimeOut(int v) { g_max_timeout = (v * 1000); }
