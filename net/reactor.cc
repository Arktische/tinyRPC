...
void Reactor::addWakeupFd() {
	...
	if ((epoll_ctl(m_epfd, op, m_wake_fd, &event)) != 0) {
		LOG(ERROR) << "epoo_ctl error, fd[" << m_wake_fd << "], errno=" << errno << ", err=" << strerror(errno) ;
	}
		...
		if (rt < 0) {
          ...
          if (ptr != nullptr) {
            ...
            if ((!(one_event.events & EPOLLIN)) && (!(one_event.events & EPOLLOUT))){
              LOG(DEBUG) << "socket [" << fd << "] occur other unknow event:[" << one_event.events << "], need unregister this socket";
              delEventInLoopThread(fd);
            } else {
...
void Reactor::addCoroutine(common::Coroutine::ptr cor, bool is_wakeup /*=true*/) {

  auto func = [cor](){
    common::Coroutine::Resume(cor.get());
  };
