...
TcpConnection::TcpConnection(net::TcpServer* tcp_svr, net::IOThread* io_thread, int fd, int buff_size, NetAddress::ptr peer_addr)
...
TcpConnection::TcpConnection(net::TcpClient* tcp_cli, net::Reactor* reactor, int fd, int buff_size, NetAddress::ptr peer_addr)
...
void TcpConnection::input() {
  ...
  while (!read_all) {
    ...
    LOG(DEBUG) << "m_read_buffer size=" << m_read_buffer->getBufferVector().size() << "rd=" << m_read_buffer->readIndex() << "wd=" << m_read_buffer->writeIndex();
    ...
    if (rt <= 0) {
      LOG(DEBUG) << "rt <= 0";
      LOG(ERROR) << "read empty while occur read event, because of peer close, sys error=" << strerror(errno) << ", now to clear tcp connection";
      clearClient();
  ...
  LOG(INFO) << "recv [" << count << "] bytes data from [" << m_peer_addr->toString() << "], fd [" << m_fd << "]";
  if (m_connection_type == ServerConnection) {
...
void TcpConnection::output() {
  while(true) {
    ...
    if (m_write_buffer->readAble() == 0) {
      LOG(DEBUG) << "app buffer of fd[" << m_fd << "] no data to write, to yiled this coroutine";
      break;
    }
    ...
    LOG(DEBUG) << "recycle write index =" << m_write_buffer->writeIndex() << ", read_index =" << m_write_buffer->readIndex() << "readable = " << m_write_buffer->readAble();
    LOG(INFO) << "send[" << rt << "] bytes data to [" << m_peer_addr->toString() << "], fd [" << m_fd << "]";
    if (m_write_buffer->readAble() <= 0) {
      // LOG(INFO) << "send all data, now unregister write event on reactor and yield Coroutine";
