...
bool Coroutine::setCallBack(std::function<void()> cb) {
  ...
  if (m_is_in_cofunc) {
    LOG(ERROR) << "this coroutine is in CoFunction";
    return false;
  }
...
void Coroutine::Yield() {
  if (t_main_coroutine == nullptr) {
    LOG(ERROR) << "main coroutine is nullptr";
    return;
  }

  if (t_cur_coroutine == t_main_coroutine) {
    LOG(ERROR) << "current coroutine is main coroutine";
    return;
  }
...
void Coroutine::Resume(Coroutine* co) {
  ...
  if (co == nullptr) {
    LOG(ERROR) << "pending coroutine is nullptr";
    return;
  }
  t_cur_coroutine = co;
